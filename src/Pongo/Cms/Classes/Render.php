...
class Render {
	...
	/**
	 * [assetLang description]
	 * @param  [type] $source     [description]
	 * @param  array  $attributes [description]
	 * @return [type]             [description]
	 */
	public function assetLang($source = null, $attributes = array())
	{
		if ( ! is_null($source) and CMSLANG != 'en')
		{
			$path = env('local') ? $this->development_path : $this->asset_path;

			$file_name = '/' . \Pongo::settings('languages.'.CMSLANG.'.file') . '.js';

			return \HTML::script($path . $source . $file_name, $attributes);
		}
	}
	...
	/**
	 * [toolsToggle description]
	 * @param  string $icon [description]
	 * @return [type]       [description]
	 */
	public function toolsToggle($icon = 'fa-cog')
	{
		$ttbutton_view = $this->view('partials.toolstoggle');
		
		$ttbutton_view['icon'] = $icon;

		return $ttbutton_view;
	}
